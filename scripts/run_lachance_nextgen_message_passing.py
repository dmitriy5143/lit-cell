...
VARIANTS = (
    "mp_gated_full",
    "mp_gated_no_velocity",
    "mp_gated_radial",
    "mp_gated_velocity_state",
    "mp_gated_c_eff_selector",
    "mp_gated_c_eff_observable",
    "mp_gated_c_eff_aux",
    "mp_gated_c_eff_no_prior",
    "mp_gated_c_eff_shuffled",
    "mp_gated_c_eff_distance_only",
    "mp_gated_c_eff_lite",
    "mp_gated_c_eff_lite_no_prior",
    "mp_gated_c_eff_lite_shuffled",
    "mp_gated_radial_c_eff_lite",
    "mp_gated_transition_full",
    "mp_gated_transition_no_prior",
    "mp_gated_transition_shuffled",
    "mp_gated_transition_sign_flip",
    "mp_gated_transition_no_velocity",
    "mp_gated_transition_no_density",
    "mp_gated_transition_no_boundary",
)
...
def edge_softmax(logits: torch.Tensor, dst: torch.Tensor, n: int) -> torch.Tensor:
    """Softmax over incoming edges for every destination node."""
    if logits.numel() == 0:
        return logits
    max_per_dst = arch.scatter_max(logits, dst, n)
    shifted = logits - max_per_dst[dst]
    exp = torch.exp(torch.clamp(shifted, -40.0, 20.0))
    denom = arch.scatter_sum(exp, dst, n)[dst].clamp_min(1e-8)
    return exp / denom


@dataclass(frozen=True)
class MPVariantConfig:
    mode: str
    prior_source: str = "correct"
    use_ceff: bool = False
    ceff_mode: str = "softmax"
    use_observability: bool = False
    aux_align_weight: float = 0.0
    multiscale: bool = False
    use_transition: bool = False
    transition_velocity: bool = True
    transition_density: bool = True
    transition_boundary: bool = True


def variant_config(variant: str) -> MPVariantConfig:
    mapping = {
        "mp_gated_full": MPVariantConfig(mode="full"),
        "mp_gated_no_velocity": MPVariantConfig(mode="no_velocity"),
        "mp_gated_radial": MPVariantConfig(mode="radial"),
        "mp_gated_velocity_state": MPVariantConfig(mode="velocity_state", multiscale=True),
        "mp_gated_c_eff_selector": MPVariantConfig(
            mode="velocity_state", use_ceff=True, multiscale=True
        ),
        "mp_gated_c_eff_observable": MPVariantConfig(
            mode="velocity_state", use_ceff=True, use_observability=True, multiscale=True
        ),
        "mp_gated_c_eff_aux": MPVariantConfig(
            mode="velocity_state",
            use_ceff=True,
            use_observability=True,
            aux_align_weight=0.015,
            multiscale=True,
        ),
        "mp_gated_c_eff_no_prior": MPVariantConfig(
            mode="velocity_state", prior_source="none", use_ceff=True, multiscale=True
        ),
        "mp_gated_c_eff_shuffled": MPVariantConfig(
            mode="velocity_state", prior_source="shuffled", use_ceff=True, multiscale=True
        ),
        "mp_gated_c_eff_distance_only": MPVariantConfig(
            mode="velocity_state", prior_source="distance", use_ceff=True, multiscale=True
        ),
        "mp_gated_c_eff_lite": MPVariantConfig(
            mode="velocity_state", use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_c_eff_lite_no_prior": MPVariantConfig(
            mode="velocity_state",
            prior_source="none",
            use_ceff=True,
            ceff_mode="gate",
        ...
        "mp_gated_c_eff_lite_shuffled": MPVariantConfig(
            mode="velocity_state",
            prior_source="shuffled",
            use_ceff=True,
            ceff_mode="gate",
            multiscale=True,
        ),
        "mp_gated_radial_c_eff_lite": MPVariantConfig(
            mode="radial", use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_transition_full": MPVariantConfig(
            mode="velocity_state", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_transition_no_prior": MPVariantConfig(
            mode="velocity_state", prior_source="none", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_transition_shuffled": MPVariantConfig(
            mode="velocity_state", prior_source="shuffled", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_transition_sign_flip": MPVariantConfig(
            mode="velocity_state", prior_source="sign_flip", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True
        ),
        "mp_gated_transition_no_velocity": MPVariantConfig(
            mode="no_velocity",
            use_transition=True,
            use_ceff=True,
            ceff_mode="gate",
            multiscale=True,
            transition_velocity=False,
        ),
        "mp_gated_transition_no_density": MPVariantConfig(
            mode="velocity_state", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True, transition_density=False
        ),
        "mp_gated_transition_no_boundary": MPVariantConfig(
            mode="velocity_state", use_transition=True, use_ceff=True, ceff_mode="gate", multiscale=True, transition_boundary=False
        ),
    }
...
class EquivariantMPDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if mode not in {"full", "no_velocity", "radial", "velocity_state"}:
            raise ValueError(f"Unknown mode: {mode}")
        if prior_source not in {"correct", "shuffled", "sign_flip", "none", "distance"}:
            raise ValueError(f"Unknown prior_source: {prior_source}")
        if ceff_mode not in {"softmax", "gate"}:
            raise ValueError(f"Unknown ceff_mode: {ceff_mode}")
        self.mode = mode
        self.prior_source = prior_source
        self.use_ceff = bool(use_ceff)
        self.ceff_mode = ceff_mode
        self.use_observability = bool(use_observability)
        self.multiscale = bool(multiscale)
        self.use_transition = bool(use_transition)
        self.transition_velocity = bool(transition_velocity)
        self.transition_density = bool(transition_density)
        self.transition_boundary = bool(transition_boundary)
        self.layers = int(layers)
        self.max_delta_norm = float(max_delta_norm)
        self._boundary_cache: dict[int, tuple[torch.Tensor, torch.Tensor]] = {}
        ...
        transition_edge_dim = 9 if self.use_transition else 0
        edge_in = 2 * hidden_dim + edge_dim + 2 + 2 + 2 + 1 + 2 + 2 + transition_edge_dim
        self.basis_count = 8 if self.use_transition else 6
        ...
        self.edge_coeff = nn.ModuleList(
            [nn.Linear(edge_hidden_dim, self.basis_count) for _ in range(self.layers)]
        )
        self.ceff_head = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(edge_hidden_dim + 8, edge_hidden_dim),
                    nn.SiLU(),
    ...
    def _reset_outputs(self) -> None:
        ...
        obs_last = self.observability_head[-1]
        assert isinstance(obs_last, nn.Linear)
        nn.init.normal_(obs_last.weight, std=1e-3)
        obs_last.bias.data.fill_(1.35)

    def _prior_values(self, graph: arch.GraphTensors) -> tuple[torch.Tensor, torch.Tensor]:
        if self.prior_source == "correct":
            return graph.c_correct, graph.force_correct
        if self.prior_source == "shuffled":
            return graph.c_shuffled, graph.force_shuffled
        if self.prior_source == "sign_flip":
            return -graph.c_correct, -graph.force_correct
        if self.prior_source == "distance":
            # `edge_features[:, 0]` is standardized distance/r_cut, so smaller is closer.
            distance_score = -graph.edge_features[:, 0:1]
            return distance_score, torch.zeros_like(distance_score)
        return torch.zeros_like(graph.c_correct), torch.zeros_like(graph.force_correct)

    def _boundary_proxy(self, graph: arch.GraphTensors) -> tuple[torch.Tensor, torch.Tensor]:
        key = id(graph.current_pos_px)
        cached = self._boundary_cache.get(key)
        if cached is not None:
            return cached
        n = graph.current_pos_px.shape[0]
        device = graph.current_pos_px.device
        if n == 0:
            empty_scalar = torch.empty((0, 1), dtype=torch.float32, device=device)
            empty_vec = torch.empty((0, 2), dtype=torch.float32, device=device)
            return empty_scalar, empty_vec
        group_map: dict[tuple[int, int], int] = {}
        group_ids = np.empty(n, dtype=np.int64)
        for i, key_pair in enumerate(zip(graph.seq_id.tolist(), graph.frame.tolist())):
            if key_pair not in group_map:
                group_map[key_pair] = len(group_map)
            group_ids[i] = group_map[key_pair]
        ...
        count = arch.scatter_sum(ones, group, g).clamp_min(1.0)
        center = arch.scatter_sum(graph.current_pos_px, group, g) / count
        vec = graph.current_pos_px - center[group]
        radius = torch.linalg.vector_norm(vec, dim=1, keepdim=True)
        max_radius = arch.scatter_max(radius, group, g)[group].clamp_min(1.0)
        boundary = torch.clamp(radius / max_radius, 0.0, 1.5)
        outward = vec / radius.clamp_min(1e-6)
        cached = (boundary, outward)
    ...
    def _edge_context(
        ...
        prior_value, force_value = self._prior_values(graph)
        transition_parts: list[torch.Tensor] = []
        density_pressure = torch.zeros_like(radial)
        boundary_basis = torch.zeros_like(radial)
        if self.use_transition:
            deg_src = torch.log1p(graph.degree[src]) / math.log(10.0)
            deg_dst = torch.log1p(graph.degree[dst]) / math.log(10.0)
            src_norm = torch.linalg.vector_norm(graph.current_velocity[src], dim=1, keepdim=True)
            dst_norm = torch.linalg.vector_norm(graph.current_velocity[dst], dim=1, keepdim=True)
            alignment = torch.sum(
                graph.current_velocity[src] * graph.current_velocity[dst],
                dim=1,
                keepdim=True,
            ) / (src_norm * dst_norm).clamp_min(1e-6)
            if not self.transition_velocity:
                alignment = torch.zeros_like(alignment)
            boundary, outward = self._boundary_proxy(graph)
            boundary_src = boundary[src]
            boundary_dst = boundary[dst]
            if not self.transition_density:
                deg_src = torch.zeros_like(deg_src)
                deg_dst = torch.zeros_like(deg_dst)
            if not self.transition_boundary:
                boundary_src = torch.zeros_like(boundary_src)
                boundary_dst = torch.zeros_like(boundary_dst)
            density_strength = deg_src + deg_dst
            density_pressure = density_strength * radial
            boundary_basis = boundary_dst * outward[dst]
            transition_parts = [
                prior_value,
                force_value,
                torch.abs(prior_value),
                deg_src,
                deg_dst,
                alignment,
                density_strength,
        ...
        if self.use_transition:
            base_list.extend([density_pressure, boundary_basis])
        bases = torch.stack(base_list, dim=1)
        return edge_input, reliability, bases, front, prior_value, force_value
    ...
    def forward(
        ...
        ceff_values: list[torch.Tensor] = []
        ceff_entropy_values: list[torch.Tensor] = []
        ...
        for layer_id in range(self.layers):
            edge_input, reliability, bases, front, prior_value, force_value = self._edge_context(graph, node)
            ...
            if self.multiscale:
                near = torch.sigmoid(1.25 * prior_value)
                far = torch.sigmoid(-1.25 * prior_value)
                coeff = coeff.clone()
                coeff[:, 0:1] = coeff[:, 0:1] * (0.65 + 0.70 * near)
                coeff[:, 1:2] = coeff[:, 1:2] * (0.65 + 0.70 * far)
                coeff[:, 2:3] = coeff[:, 2:3] * (0.70 + 0.60 * torch.abs(front))
            message = torch.sum(coeff[:, :, None] * bases, dim=1)
            if self.use_ceff:
                src_speed = graph.speed_norm[graph.src]
                dst_speed = graph.speed_norm[graph.dst]
                ceff_input = torch.cat(
                    [
                        edge_hidden,
                        prior_value,
                        force_value,
                ...
                learned = self.ceff_head[layer_id](ceff_input)
                anchor = 0.35 * prior_value + 0.10 * force_value
                if self.ceff_mode == "softmax":
                    ceff = edge_softmax(learned + anchor, graph.dst, n)
                else:
                    # Lightweight c_eff: preserve the base aggregation and only
                    # apply a bounded, state-aware edge gate. This is deliberately
                    # less expressive than incoming-edge softmax, because the
                    # full softmax variant was too disruptive on MDCK Edge.
                    ceff = 0.35 + 1.30 * torch.sigmoid(learned + anchor)
            else:
                ceff = torch.ones_like(edge_gate)
            edge_weight = reliability * edge_gate * ceff
            weighted_vec = edge_weight * message
            weighted_hidden = edge_weight * edge_hidden
            effective = arch.scatter_sum(edge_weight, graph.dst, n).clamp_min(1e-4)
            ...
            ceff_entropy_values.append(-(ceff.clamp_min(1e-8).log() * ceff))
                ...
                self.observability_head(
                    torch.cat(
                        [
                            ...
                            graph.speed_norm,
                            log_degree,
                            torch.linalg.vector_norm(flow_pred, dim=1, keepdim=True),
                        ],
                        dim=1,
                    )
                )
            )
            node_gate = node_gate * obs
        else:
            obs = torch.ones_like(node_gate)
        ...
        return delta, flow_gate, {
            ...
            "ceff": torch.cat(ceff_values, dim=0),
            "ceff_entropy": torch.cat(ceff_entropy_values, dim=0),
...
def train_mp_decoder(
    ...
    *,
    ...
    model = EquivariantMPDecoder(
        ...
        use_ceff=cfg.use_ceff,
        ceff_mode=cfg.ceff_mode,
        use_observability=cfg.use_observability,
        multiscale=cfg.multiscale,
        use_transition=cfg.use_transition,
        transition_velocity=cfg.transition_velocity,
        transition_density=cfg.transition_density,
        transition_boundary=cfg.transition_boundary,
    ...
    for epoch in range(int(epochs)):
        ...
        if cfg.aux_align_weight > 0.0:
            residual = (
                train.y_norm.detach()
                - train_enc.self_pred.detach()
                - train_enc.flow_pred.detach()
            )
            active_res = residual[train.target_valid]
            denom = (
                torch.linalg.vector_norm(active_delta, dim=1)
                * torch.linalg.vector_norm(active_res, dim=1)
            ).clamp_min(1e-6)
            cosine = torch.sum(active_delta * active_res, dim=1) / denom
            loss = loss + float(cfg.aux_align_weight) * torch.mean(1.0 - cosine)
        loss.backward()
...
def evaluate_mp(
    ...
    ceff = diag["ceff"].detach().cpu().numpy().reshape(-1)
    ceff_entropy = diag["ceff_entropy"].detach().cpu().numpy().reshape(-1)
    edge_gate = diag["edge_gate"].detach().cpu().numpy().reshape(-1)
    observability = diag["observability"].detach().cpu().numpy().reshape(-1)
    ceff_max = np.nan
    if len(ceff):
        # The tensor concatenates layers; max is reported as a global selection sharpness proxy.
        ceff_max = float(np.nanmax(ceff))
    metrics.update(
        {
            ...
            "observability_mean": float(np.mean(observability[graph.target_valid.detach().cpu().numpy()])),
            "observability_std": float(np.std(observability[graph.target_valid.detach().cpu().numpy()])),
            ...
            "edge_gate_mean": float(np.mean(edge_gate)),
            "effective_degree_mean": float(diag["effective_degree"][graph.target_valid].mean().cpu()),
            "ceff_mean": float(np.mean(ceff)) if len(ceff) else float("nan"),
            "ceff_max": ceff_max,
            "ceff_entropy_mean": float(np.mean(ceff_entropy)) if len(ceff_entropy) else float("nan"),
        }
    )
    ...
    out.update(
        {
            ...
            "observability_std": 0.0,
            "ceff_mean": 0.0,
            "ceff_max": 0.0,
            "ceff_entropy_mean": 0.0,
        }
    )
    return out
